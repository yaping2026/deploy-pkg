...
async function sendMonthlyReport(targetGroupId, dryRun) {
  ...
  for (const group of groups) {
    ...
    // 不达标：某成员用完2次补卡券后还有缺卡（即总缺卡 > 2次，不可补救缺卡 >= 1次）
    // 补了卡的算正常打卡，月度报表中所有缺卡都是无任何打卡记录的天数
    const MAKEUP_VOUCHERS = 2; // 每月补卡券2次
    const failedMembers = memberStats.filter(m => m.missedCount > MAKEUP_VOUCHERS);
  ...
  if (nonCompliantGroups.length > 0) {
    ...
    for (const r of nonCompliantGroups) {
      ...
      for (const m of r.failedMembers) {
        const irrecoverable = m.missedCount - 2; // 超出补卡券的不可补救缺卡次数
        msg += '  · ' + m.member.name + '  缺卡 ' + m.missedCount + ' 次（超补卡券 ' + irrecoverable + ' 次）\n';
      }
      msg += '\n';
    }
  }
...
function scheduleMonthlyReport() {
  cron.schedule('0 6 2 * *', async () => {
    ...
    // 去重检查：数据库 _cronSent.monthly 标记
    const monthInfo = getPreviousMonthInfo();
    try {
      const data = await db.read();
      if (data._cronSent && data._cronSent.monthly === monthInfo.yearMonth) {
        console.log('[月报] ⚠️ ' + monthInfo.yearMonth + ' 月报已发送过，跳过');
        return;
      }
    } catch (e) {
      console.error('[月报] 数据库读取失败:', e.message);
    }
    ...
    try {
      ...
      // 标记数据库
      try {
        const markData = await db.read();
        if (!markData._cronSent) markData._cronSent = {};
        markData._cronSent.monthly = monthInfo.yearMonth;
        await db.write(markData);
        console.log('[月报] ✅ 已标记数据库：' + monthInfo.yearMonth + ' 月报已发送');
      } catch (e) {
        console.error('[月报] ⚠️ 标记数据库失败:', e.message);
      }
    } catch (e) {
...
function startAll() {
  ...
  console.log('[version] reading-checkin v2026-08-06-v5.11 (根治重复发送+0%@all：tryAcquireCronLock原子锁替代三重保险、readFresh加fresh标记防旧数据发错误消息)');
  console.log('[定时任务] 已启动: 06:00日报+内容 | 22:00提醒 | 月报(每月2号6点) | 海报补偿(5min间隔,5min宽限期)');
}

module.exports = { startAll, getTodayStr, getYesterdayStr, getWeekdayName, sendMorning, sendMorningContent, sendDailyReport, sendEveningReminder, resendMissingPosters, sendMonthlyReport, getPreviousMonthInfo };
