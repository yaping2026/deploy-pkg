...
async function sendMonthlyReport(targetGroupId, dryRun) {
  ...
  for (const group of groups) {
    ...
    for (const member of members) {
      ...
      if (memberEnd && memberEnd <= startDate) continue; // 退出日期早于上月1号 → 上月不统计

      // 有效日期：从 max(memberStart, monthStart) 到 endDate（退出者截止到 leftDate 前一天）
      const effectiveStart = memberStart > startDate ? memberStart : startDate;
      const eligibleDates = dates.filter(d => d >= effectiveStart && d <= endDate && (!memberEnd || d < memberEnd));
      ...
      // v5.12.4 用户新标准：个人不达标 = 统计周期内存在任意一天未补上的缺卡。
      // 补卡只能在缺卡次日（且不跨月）进行；月报统计时(下月2号)上月所有缺卡
      // 均已过补卡期限，属于"没办法补卡的情况"→ 直接判不达标，不管还剩几张补卡券。
      // 旧逻辑 v5.12.3 用"剩余补卡券"抵扣缺卡数，会把"缺1次但券没用过"的人误判达标。
      const makeupUsed = memberCheckins.filter(c =>
        c.type === 'makeup' || c.type === 'makeup-read' || c.type === 'makeup-checkin'
      ).length;
      // 不可补救缺卡 = 所有未补上的缺卡（统计时点全部已过补卡窗口）
      const irrecoverableMisses = missedCount;
  ...
  if (nonCompliantGroups.length > 0) {
    ...
    for (const r of nonCompliantGroups) {
      ...
      for (const m of r.failedMembers) {
        // v5.12.4：所有未补缺卡均已过补卡期限，直接展示"已过补卡期限"
        const irrecoverable = m.irrecoverableMisses;
        msg += '  · ' + m.member.name + '  缺卡 ' + m.missedCount + ' 次（已过补卡期限，不可补救 ' + irrecoverable + ' 次）\n';
      }
      msg += '\n';
    }
  }
...
async function sendDailyReport(targetGroupId, dryRun) {
  ...
  const groupReports = [];
  for (const group of groups) {
    // v5.13: 已停止打卡（leftDate）的成员不参与日报统计
    const members = data.members.filter(m => m.groupId === group.id && !m.leftDate);
...
async function sendEveningReminder(targetGroupId) {
  ...
  try {
    ...
    for (const group of groups) {
      // v5.13: 已停止打卡（leftDate）的成员不参与晚报提醒
      const members = data.members.filter(m => m.groupId === group.id && !m.leftDate);
...
function startAll() {
  ...
  console.log('[version] reading-checkin v2026-08-28-v5.13 (停止打卡: 成员leftDate标记退出, 退出当天起不再要求打卡; 月报/日报/晚报按有效统计期; 达标判定=存在任意未补缺卡即不达标)');
