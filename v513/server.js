...
const APP_VERSION = '2026-08-28-v5.13'; // 停止打卡: 成员leftDate标记退出, 退出当天起不再要求打卡; 月报/日报/晚报/打卡名单/换组按活跃成员; 后台报表统计期裁剪含退出前缺卡 // 前版: v5.12.4达标判定(存在任意未补缺卡即不达标) + v5.12.2月报复发修复 + v5.12季度换组 + SSL监听443
...
app.get('/api/groups', async (req, res) => {
  ...
  const groups = data.groups.map(g => ({
    ...g,
    memberCount: data.members.filter(m => m.groupId === g.id && !m.leftDate).length // v5.13: 只统计未停止打卡的成员
  }));
...
// v5.13 停止打卡：成员退出项目（保留历史数据，标记leftDate后不再参与打卡/日报/晚报/换组，
// 历史月份的报表统计仍按"当时的组"包含其退出前的缺卡记录）
app.post('/api/members/:id/stop', requireAdmin, async (req, res) => {
  const { data } = await db.readFresh();
  const member = data.members.find(m => m.id === parseInt(req.params.id));
  if (!member) return res.status(404).json({ ok: false, msg: '成员不存在' });
  if (member.leftDate) return res.json({ ok: false, msg: '该成员已于 ' + member.leftDate + ' 停止打卡' });
  member.leftDate = scheduler.getTodayStr();
  await db.write(data);
  res.json({ ok: true, member });
});

// v5.13 恢复打卡：清除 leftDate，成员重新参与打卡/日报/统计
app.post('/api/members/:id/resume', requireAdmin, async (req, res) => {
  const { data } = await db.readFresh();
  const member = data.members.find(m => m.id === parseInt(req.params.id));
  if (!member) return res.status(404).json({ ok: false, msg: '成员不存在' });
  if (!member.leftDate) return res.json({ ok: false, msg: '该成员当前为正常打卡状态' });
  delete member.leftDate;
  await db.write(data);
  res.json({ ok: true, member });
});
...
app.post('/api/regroup', requireAdmin, async (req, res) => {
  try {
    ...
    for (const mv of moves) {
      ...
      // v5.13: 已停止打卡的成员不参与换组
      if (member.leftDate) { results.push({ memberId: mv.memberId, name: member.name, ok: false, msg: '已停止打卡，不参与换组' }); continue; }
...
app.get('/api/checkins/today/:groupId', async (req, res) => {
  ...
  const groupId = parseInt(req.params.groupId);
  // v5.13: 已停止打卡（leftDate）的成员不出现打卡名单，不计入今日统计
  const members = data.members.filter(m => m.groupId === groupId && !m.leftDate);
...
app.post('/api/checkins', async (req, res) => {
  ...
  for (let retry = 0; retry < 3; retry++) {
    ...
    if (!member) return res.status(404).json({ ok: false, msg: '成员不存在' });
    // v5.13: 已停止打卡的成员拒绝提交（身份选择名单已过滤，此兜底防止旧cookie/手工调用）
    if (member.leftDate) return res.status(400).json({ ok: false, msg: '您已停止打卡，如需恢复请联系管理员' });
    ...
    const groupMembers = data.members.filter(m => m.groupId === member.groupId && !m.leftDate);
...
app.post('/api/checkins/makeup', async (req, res) => {
  ...
  if (!member) return res.status(404).json({ ok: false, msg: '成员不存在' });
  // v5.13: 已停止打卡的成员拒绝补卡
  if (member.leftDate) return res.status(400).json({ ok: false, msg: '您已停止打卡，无法补卡' });
...
async function generateReportData(startDate, endDate, groupId) {
  ...
  for (const group of targetGroups) {
    ...
    const memberStats = members.map(member => {
      ...
      // v5.13 停止打卡：leftDate 当天起不再要求打卡（有效截止 = leftDate 前一天）
      const memberEnd = member.leftDate || null;
      
      const memberCheckins = groupCheckins.filter(c => c.memberId === member.id);
      
      const dailyStatus = {};
      for (const date of dates) {
        if (date < memberStart || (memberEnd && date >= memberEnd)) {
          // 加入前 / 退出后：显示"—"，不计入任何统计
          dailyStatus[date] = '—';
        } else {
      ...
      // 累计未打卡天数（只统计加入后的✗，退出者截止到 leftDate 前一天）
      const totalMissed = dates.filter(dd => dd >= memberStart && dd <= today && (!memberEnd || dd < memberEnd) && dailyStatus[dd] === '✗').length;

      // 有效天数 = 加入后到今天的应打卡天数（退出者截止到 leftDate 前一天）
      const eligibleDays = dates.filter(dd => dd >= memberStart && dd <= today && (!memberEnd || dd < memberEnd)).length;
      ...
      // === 按考核标准判断预警 ===
      // v5.12.4 规则：个人不达标 = 存在任意一天"没办法补卡"的缺卡（不可补救缺卡>=1次），不管剩余补卡券
      // 小组不达标 = 组内任何一人不达标 → 整组不达标
      // 补卡只能补昨天：昨天之前的✗是永久性缺卡；昨天的✗若有券还能补
      const yesterday = scheduler.getYesterdayStr();
      const remainingMakeup = 2 - makeupUsed; // 剩余补卡券
      // 永久性✗：只统计加入后的、昨天之前的✗（退出者截止到 leftDate 前一天）
      const permanentMisses = dates.filter(dd => dd >= memberStart && dd < yesterday && (!memberEnd || dd < memberEnd) && dailyStatus[dd] === '✗').length;
      const fixableMisses = (memberStart <= yesterday && (!memberEnd || yesterday < memberEnd) && dailyStatus[yesterday] === '✗') ? 1 : 0;
      ...
      // alertLevel: 0=正常, 1=风险(缺卡但昨天当天还有券可补), 2=不达标(不可补救缺卡>=1)
      let alertLevel = 0;
      let alertReason = [];
      let qualified = true;
      ...
      if (totalMissed === 0) {
        ...
        // v5.12.4：存在任意一天不可补救缺卡 → 不达标（1次即判，不再要求>=2）
        alertLevel = 2;
        alertReason.push('缺卡' + totalMissed + '次（不可补救' + unfixableMisses + '次，已过补卡期限）');
        qualified = false;
      } else {
        // 全部缺卡都还能补救（仅昨天缺卡且有券）→ 风险
        alertLevel = 1;
        alertReason.push('缺卡' + totalMissed + '次（昨日缺卡尚可补卡），剩余补卡券' + remainingMakeup + '张');
      }
...
app.post('/api/test/monthly-report', async (req, res) => {
  ...
  if (confirm === true) {
    // 真发送：走原子锁（一个月只发一次，与cron共用锁）
    res.json({ ok: true, msg: '月报发送任务已启动（若本月已发送过将自动跳过）' });
    scheduler.trySendMonthlyReport().then(result => {
      if (result && !result.sent) {
        console.log('[test/monthly-report] 月报未发送:', result.reason);
      }
    }).catch(err => {
