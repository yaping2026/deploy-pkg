#!/bin/bash
# ============================================================
# 一键迁移脚本：从 deploy-pkg 仓库拉取所有 content/ + audio/ 文件
# 适用：本地存储模式 (LOCAL_ONLY_MODE)
# 用法：curl -sL ... | bash
# ============================================================
set -e

APP=/opt/reading-checkin
CONTENT_DIR=$APP/content
AUDIO_DIR=$APP/data/audio
LOG=/var/log/migrate_files.log
LIST=/tmp/file_list.json

GREEN='\033[0;32m'
RED='\033[0;31m'
YELLOW='\033[1;33m'
CYAN='\033[0;36m'
NC='\033[0m'

ok()   { echo -e "${GREEN}[OK]${NC} $1" | tee -a $LOG; }
fail() { echo -e "${RED}[FAIL]${NC} $1" | tee -a $LOG; exit 1; }
step() { echo -e "${YELLOW}[$1]${NC} $2" | tee -a $LOG; }

: > $LOG
START_TIME=$(date +%s)
echo "迁移启动: $(date '+%Y-%m-%d %H:%M:%S')" >> $LOG

step "1/5" "获取文件清单..."
curl -sL --connect-timeout 30 --max-time 120 \
  "https://api.github.com/repos/yaping2026/deploy-pkg/git/trees/main?recursive=1" \
  -o $LIST || fail "无法获取文件清单"

node -e "
const fs=require('fs');
const j=JSON.parse(fs.readFileSync('$LIST','utf8'));
const trees=j.tree||[];
const contentFiles=trees.filter(t=>t.path.startsWith('content/')).map(t=>t.path.replace(/^content\//,''));
const audioFiles=trees.filter(t=>t.path.startsWith('audio/')).map(t=>t.path.replace(/^audio\//,''));
fs.writeFileSync('/tmp/content_list.json',JSON.stringify(contentFiles));
fs.writeFileSync('/tmp/audio_list.json',JSON.stringify(audioFiles));
console.log('  content文件数:',contentFiles.length);
console.log('  audio文件数:',audioFiles.length);
" || fail "解析文件清单失败"

step "2/5" "创建目录..."
mkdir -p $CONTENT_DIR $AUDIO_DIR
ok "目录就绪"

# 通用下载函数：GitHub raw 优先，jsDelivr 兜底
cdn_download_file() {
  local subDir=$1 filename=$2 outPath=$3
  for try in 1 2; do
    if curl -fsSL --connect-timeout 30 --max-time 180 \
      "https://raw.githubusercontent.com/yaping2026/deploy-pkg/main/${subDir}/${filename}" \
      -o "$outPath.tmp" 2>/dev/null; then
      if [ -s "$outPath.tmp" ] && [ $(stat -c%s "$outPath.tmp" 2>/dev/null || echo 0) -gt 100 ]; then
        mv "$outPath.tmp" "$outPath"
        return 0
      fi
    fi
    rm -f "$outPath.tmp"
    sleep 1
  done
  if curl -fsSL --connect-timeout 30 --max-time 180 \
    "https://gcore.jsdelivr.net/gh/yaping2026/deploy-pkg@main/${subDir}/${filename}" \
    -o "$outPath.tmp" 2>/dev/null; then
    if [ -s "$outPath.tmp" ] && [ $(stat -c%s "$outPath.tmp" 2>/dev/null || echo 0) -gt 100 ]; then
      mv "$outPath.tmp" "$outPath"
      return 0
    fi
  fi
  rm -f "$outPath.tmp"
  return 1
}

step "3/5" "迁移 content/ 文件（PDF/图片，共36个，快）..."
mapfile -t CONTENT_FILES < <(node -e "console.log(JSON.parse(require('fs').readFileSync('/tmp/content_list.json','utf8')).join('\n'))")
TOTAL=${#CONTENT_FILES[@]}
COUNT=0
SUCCESS=0
SKIP=0
FAIL=0

for filename in "${CONTENT_FILES[@]}"; do
  COUNT=$((COUNT+1))
  [ -z "$filename" ] && continue
  OUT=$CONTENT_DIR/$filename
  if [ -f "$OUT" ] && [ $(stat -c%s "$OUT" 2>/dev/null || echo 0) -gt 100 ]; then
    SKIP=$((SKIP+1))
    echo "  [$COUNT/$TOTAL] 跳过: $filename" >> $LOG
  else
    if cdn_download_file "content" "$filename" "$OUT"; then
      SUCCESS=$((SUCCESS+1))
      SIZE=$(stat -c%s "$OUT" 2>/dev/null || echo 0)
      echo -e "  ${GREEN}[$COUNT/$TOTAL]${NC} ✅ $filename (${SIZE}B)" | tee -a $LOG
    else
      FAIL=$((FAIL+1))
      echo -e "  ${RED}[$COUNT/$TOTAL]${NC} ❌ $filename" | tee -a $LOG
    fi
  fi
done

CONTENT_OK=$(ls $CONTENT_DIR 2>/dev/null | wc -l)
ok "content完成: $CONTENT_OK 个文件（新下载 $SUCCESS，跳过 $SKIP，失败 $FAIL）"

step "4/5" "迁移 audio/ 录音（共159个，慢）..."
mapfile -t AUDIO_FILES < <(node -e "console.log(JSON.parse(require('fs').readFileSync('/tmp/audio_list.json','utf8')).join('\n'))")
TOTAL=${#AUDIO_FILES[@]}
COUNT=0
SUCCESS=0
SKIP=0
FAIL=0
echo "  共 $TOTAL 个录音，开始批量下载..." | tee -a $LOG

for filename in "${AUDIO_FILES[@]}"; do
  COUNT=$((COUNT+1))
  [ -z "$filename" ] && continue
  OUT=$AUDIO_DIR/$filename
  if [ -f "$OUT" ] && [ $(stat -c%s "$OUT" 2>/dev/null || echo 0) -gt 1000 ]; then
    SKIP=$((SKIP+1))
    echo "  [$COUNT/$TOTAL] 跳过: $filename" >> $LOG
  else
    if cdn_download_file "audio" "$filename" "$OUT"; then
      SUCCESS=$((SUCCESS+1))
      if [ $((COUNT % 20)) -eq 0 ]; then
        echo -e "  ${GREEN}进度 [$COUNT/$TOTAL]${NC} ✅已下 $SUCCESS 失败 $FAIL" | tee -a $LOG
      fi
    else
      FAIL=$((FAIL+1))
      if [ $((COUNT % 10)) -eq 0 ]; then
        echo -e "  ${RED}进度 [$COUNT/$TOTAL]${NC} ❌已下 $SUCCESS 失败 $FAIL" | tee -a $LOG
      fi
    fi
  fi
done

AUDIO_OK=$(ls $AUDIO_DIR 2>/dev/null | wc -l)
ok "audio完成: $AUDIO_OK 个文件（新下载 $SUCCESS，跳过 $SKIP，失败 $FAIL）"

END_TIME=$(date +%s)
DURATION=$((END_TIME - START_TIME))

step "5/5" "最终统计..."
echo "================================" | tee -a $LOG
echo "content/ 目录: $(ls $CONTENT_DIR 2>/dev/null | wc -l) 个文件" | tee -a $LOG
echo "audio/ 目录:  $(ls $AUDIO_DIR 2>/dev/null | wc -l) 个文件" | tee -a $LOG
echo "总耗时: $((DURATION/60))分$((DURATION%60))秒" | tee -a $LOG
echo "================================" | tee -a $LOG

ok "迁移全部完成！刷新页面验证"
